...
$diasAtrasos = isset($_GET['diasAtrasos']) ? (string)$_GET['diasAtrasos'] : '30';
...
if ($tipo == 1) { // servidor
  if ($diasAtas === 'ano') {
    $anoAtual = date('Y');
    $resAtas = $conexao->query("SELECT a.*, 
  ...
  if ($diasAtas === 'ano') {
    ...
    $stmt = $conexao->prepare("SELECT a.*, 
            ...
            LEFT JOIN PARTICIPANTES p ON a.idATA = p.idAta
            LEFT JOIN USUARIO u ON p.idUSUARIO = u.idUSUARIO
            WHERE p.idUSUARIO = ? AND YEAR(a.data) = ?
            GROUP BY a.idATA
            ORDER BY a.data DESC");
...
if ($tipo != 3) {
  if ($searchAluno !== '') {
    if ($diasAtrasos === 'ano') {
      $anoAtual = date('Y');
      $stmt = $conexao->prepare("SELECT a.idATRASO, u.nome AS aluno, a.data, a.motivo
                FROM ATRASO a
  ...
  if ($diasAtrasos === 'ano') {
    $anoAtual = date('Y');
    $stmt = $conexao->prepare("SELECT idATRASO, data, motivo FROM ATRASO WHERE idAluno = ? AND YEAR(data) = ? ORDER BY data DESC");
    $stmt->bind_param("ii", $idUsuario, $anoAtual);
  } else {
    $dataLimite = date('Y-m-d', strtotime("-$diasAtrasos days"));
    $stmt = $conexao->prepare("SELECT idATRASO, data, motivo FROM ATRASO WHERE idAluno = ? AND data >= ? ORDER BY data DESC");
    $stmt->bind_param("is", $idUsuario, $dataLimite);
  }
  $stmt->execute();
  $resAtrasosRecentes = $stmt->get_result();
  ...
  <div class="container">
    ...
    <?php if ($tipo == 1): ?>
      <div class="d-flex justify-content-center mb-5 flex-wrap botao-grande-gap">
        <a href="../ATAS/cadastrar_Ata.php" class="btn btn-success btn-lg px-4"><img src="../assets/img/ata.svg"
            class="img-btn"> Registrar ATA</a>
        <a href="../ATRASOS/atrasos.php" class="btn btn-warning btn-lg px-4 text-white"><img
            src="../assets/img/atraso.svg" class="img-btn"> Registrar Atraso</a>
      </div>
    <?php endif; ?>
          ...
          <?php while ($ata = $resAtas->fetch_assoc()): ?>
            <a href="../ATAS/cadastrar_ata.php?editar=<?= $ata['idATA'] ?>" class="text-decoration-none text-dark">
              <div class="ata mb-3 p-3 hover-ata">
                ...
                <p class="text-truncate"><?= strip_tags($ata['anotacoes']) ?></p>
                <small class="text-muted">
                  <img src="../assets/img/data.svg" class="img-btn"> <?= date('d/m/Y H:i', strtotime($ata['data'])) ?><br>
                  <?php if (isset($ata['qtd_participantes'])): ?>
                    <img src="../assets/img/participantes.svg" class="img-btn"> <?= $ata['qtd_participantes'] ?>
                    participantes<br>
      ...
      <div class="col-md-6">
        <div class="d-flex justify-content-between align-items-center mb-2">
          ...
          <form method="get" class="d-flex align-items-center gap-2 mb-3">
            ...
            <?php if ($tipo != 3): ?>
              <input type="text" name="searchAluno" class="form-control form-control-sm" placeholder="Pesquisar aluno..."
        ...
        <?php else: ?>
          <div class="alert alert-info text-center">Nenhum atraso registrado nos últimos
            <?= htmlspecialchars($diasAtrasos) ?> dias.</div>
        <?php endif; ?>
      </div>
    </div>
  </div>
